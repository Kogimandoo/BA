'''
Combine the 06:00 and 18:00 npy files, calculate their median, and compare it with the first day
'''
...
OUTPUT_IMAGE = OUTPUT_DIR / "compare5_no_smooth.png"
...
# Function that loads multiple depth npy files captured on the same day
# and creates one representative depth image for that date
def load_daily_median_depth(
    ...
    for file in date_files:
        if file not in depth_cache:
            depth = load_depth(file)

            # Use the original depth data directly without applying smooth_depth()
            depth_cache[file] = depth
...
def create_daily_compare_cell(
    date: str,
    date_files: list[Path],
    baseline_depth: np.ndarray,
    current_daily_depth: np.ndarray,
    roi_mask: np.ndarray,
    min_depth: float,
    max_depth: float,
    baseline_date: str
    ...
    image = create_change_overlay(
        ...
        current_depth=current_daily_depth,
        roi_mask=roi_mask,
        min_depth=min_depth,
        max_depth=max_depth
    )
...
def create_legend(width: int, baseline_date: str) -> np.ndarray:
    legend_height = 105
    legend = np.full((legend_height, width, 3), 255, dtype=np.uint8)

    add_text(legend, "Daily median growth comparison - no smooth_depth", 10, 25, 0.55)
    add_text(legend, f"Baseline date: {baseline_date}", 10, 55, 0.45)
    add_text(legend, "Red: distance increased / Green: distance decreased", 10, 80, 0.45)
...
def main():
    ...
    files = find_depth_files()

    grouped_by_date = group_files_by_date(files)

    depth_cache = {}
    daily_depths_by_date = {}

    for date, date_files in grouped_by_date.items():
        daily_depths_by_date[date] = load_daily_median_depth(
            date_files=date_files,
            depth_cache=depth_cache
        )

    all_dates = sorted(daily_depths_by_date.keys())

    if len(all_dates) == 0:
        raise RuntimeError("No valid dates found.")

    baseline_date = all_dates[0]
    baseline_depth = daily_depths_by_date[baseline_date]
    ...
    for date in all_dates:
        date_files = grouped_by_date[date]
        current_daily_depth = daily_depths_by_date[date]

        cell = create_daily_compare_cell(
            date=date,
            date_files=date_files,
            baseline_depth=baseline_depth,
            current_daily_depth=current_daily_depth,
            roi_mask=roi_mask,
            min_depth=min_depth,
            max_depth=max_depth,
        ...
        label_area = np.full(
            (CELL_HEIGHT, DATE_LABEL_WIDTH, 3),
            255,
            dtype=np.uint8
        )
    ...
    legend = create_legend(final_image.shape[1], baseline_date)
    final_image = np.vstack([legend, final_image])

    cv2.imwrite(str(OUTPUT_IMAGE), final_image)

    print("Daily median vs first day comparison created successfully.")
    print("smooth_depth() was not applied.")
