'''
Compare with the immediately previous data
'''
...
OUTPUT_IMAGE = OUTPUT_DIR / "compare2.png"
...
def main():
    ...
    # The first file has no previous reference, so compare it with itself or display it as having no change
    ...
    for date, date_files in grouped.items():
        ...
        for file in date_files:
            current_depth = load_depth(file)

            if previous_depth is None:
                # The first image has no comparison target, so compare it with itself
                baseline_depth = current_depth
                baseline_file = file
            else:
                # Compare with the immediately previous result
                baseline_depth = previous_depth
                baseline_file = previous_file
            ...
            # Store the current file as the previous file for the next comparison
            previous_file = file
